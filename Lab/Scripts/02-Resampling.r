...
## Simulate CORRELATED Data Using MVTNORM
	##basic idea: simulate rnorm vectors, & multiply by covariance matrix
...
t.rand.vec<-NULL
t.rand.vec<-rbind(t.rand.vec,t.obs)
for(i in 1:permute){
  ###Shuffle Data
	y.rand<-sample(bumpus.data$TL)  #NOTE: notation works ONLY for single variable 
  ###Run analysis on random data
	t.rand<-t.test(formula=y.rand~sex)$statistic[[1]]
	t.rand.vec<-rbind(t.rand.vec,t.rand)
  ###Significance assessment
	P.2tailed<-if(abs(t.rand)>=abs(t.obs)) (P.2tailed+1) else P.2tailed
	P.1tailed<-if(t.rand<=t.obs) (P.1tailed+1) else P.1tailed  ##<= b/c negative t.obs
}  #end permute
P.1tailed<-P.1tailed/(permute+1)
P.2tailed<-P.2tailed/(permute+1)
P.1tailed
P.2tailed
####Plot
